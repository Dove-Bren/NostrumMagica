...
public class LayerArcaneWolfRunes extends LayerRenderer<EntityArcaneWolf, ModelArcaneWolf> {
	...
	@Override
	public void render(EntityArcaneWolf wolf, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
		if (!wolf.isInvisible()) {
			...
			wolfRenderer.getEntityModel().render(wolf, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scale);
		}
	}
