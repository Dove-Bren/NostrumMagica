...
public class SpellMortarRenderer extends EntityRenderer<SpellMortarEntity> {
	...
	public SpellMortarRenderer(EntityRendererProvider.Context renderManager, float scale) {
		super(renderManager);
		this.scale = scale;
	}

	@Override
	public ResourceLocation getTextureLocation(SpellMortarEntity entity) {
		return LOC_TEXT;
	}
	
	@Override
	public void render(SpellMortarEntity entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
		...
		VertexConsumer buffer = bufferIn.getBuffer(NostrumRenderTypes.GetBlendedEntity(getTextureLocation(entityIn), true));
