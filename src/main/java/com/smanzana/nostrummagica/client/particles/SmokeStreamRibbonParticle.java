...
import com.smanzana.nostrummagica.client.particles.ribbon.RibbonRenderTypes.LitTexturedRibbonRenderType;
...
public class SmokeStreamRibbonParticle extends RibbonParticle implements IMotionParticle<SmokeStreamRibbonParticle> {
	
	protected final ParticleTargetMotion motion;
	
	public SmokeStreamRibbonParticle(ClientLevel worldIn, double x, double y, double z, float red, float green, float blue, float alpha, int lifetime) {
		super(worldIn, x, y, z, red, green, blue, alpha, lifetime, SmokeStreamRibbonParticle::MakeSegment);
	...
	protected static @Nullable ISegment MakeSegment(RibbonParticle particle, ClientLevel worldIn, Vec3 worldPos, EmitterData emitter, double distanceFromLast, float ticksFromLast) {
		return ((SmokeStreamRibbonParticle) particle).makeSegment(worldIn, worldPos, emitter, distanceFromLast, ticksFromLast);
	}
	...
	protected @Nullable ISegment makeSegment(ClientLevel worldIn, Vec3 worldPos, EmitterData emitter, double distanceFromLast, float ticksFromLast) {
		...
		return new SmokeStreamRibbonSegment(worldPos, emitter.emitterColor, .25f, (float) (emitter.ribbonSpawnLength % 1024), age, emitter.partialTicks, 20, emitter.emitterLifetime);
	}
	...
	protected static final class SmokeStreamRibbonSegment extends LifetimeSegment implements CameraFacingSegmentMixin, FadingSegmentMixin {

		private final StaticSegment segmentWrapper;
		
		public SmokeStreamRibbonSegment(Vec3 position, Color color, float width, float v, int startTicks, float partialTicks, int lifetimeTicks, int particleLifetime) {
			super(startTicks, lifetimeTicks, particleLifetime, partialTicks, true);
		...
		public Vec3 getPosition(Camera camera, EmitterData emitter) {
			return segmentWrapper.getPosition(camera, emitter)
					//.add(0, this.getLifeProgress(emitter) * .25, 0)
					;
		}

		@Override
		public float getU(Camera camera, EmitterData emitter, SegmentData ribbonData, boolean leftEdge) {
			// Don't 'stretch' texture when it's not full. Instead calculate the U that would be at that offset
			final float widthNorm = this.getWidth(camera, emitter, ribbonData) / this.segmentWrapper.getWidth(camera, emitter, ribbonData); // 0-1f
			return .5f // middle by default
					+ (leftEdge ? -1 : 1) * (.5f) * widthNorm;
		}
		...
		public float getV(Camera camera, EmitterData emitter, SegmentData ribbonData) {
			final float period = 4f; // controls speed of texturing
			final float offset = ((float) (emitter.emitterAge  % period) / period);
			return offset + segmentWrapper.v();
		}
	}

	@Override
	public SmokeStreamRibbonParticle setGravityStrength(float strength) {
		gravity = strength;
		return this;
	}

	@Override
	public SmokeStreamRibbonParticle setMotion(double xVelocity, double yVelocity, double zVelocity) {
		this.xd = xVelocity;
		this.yd = yVelocity;
		this.zd = zVelocity;
		return this;
	...
	@Override
	public SmokeStreamRibbonParticle setPosition(double x, double y, double z) {
		this.setPos(x, y, z);
		return this;
	}
	
	private static final LitTexturedRibbonRenderType RENDER_TYPE = new LitTexturedRibbonRenderType(NostrumMagica.Loc("textures/particle/smoke_stream.png"));
	...
	public static final SmokeStreamRibbonParticle MakeParticle(ClientLevel world, SpriteSet spites, SpawnParams params) {
		SmokeStreamRibbonParticle particle = null;
		...
		for (int i = 0; i < params.count; i++) {
			...
			particle = new SmokeStreamRibbonParticle(world, spawnX, spawnY, spawnZ, colors[0], colors[1], colors[2], colors[3], lifetime);
