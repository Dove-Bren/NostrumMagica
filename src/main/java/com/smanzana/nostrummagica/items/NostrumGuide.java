...
public class NostrumGuide extends Item implements GuiBook {
	...
	public BookScreen getScreen(Object userdata) {
		...
		pages.add(new TitlePage("Pilgrimage of Discovery", true));
		pages.add(new PlainTextPage("  The Pilgrimage of Discovery is performed to unlock the magic potential in the player. The ritual requires the player to travel far and wide in search of special altars. In order to unlock magic and get started, you must locate a shrine to an element. Deep in the shrine is an obelisk"));
		pages.add(new PlainTextPage("which can be interacted with to provide insight into the element. Once you've gained knowledge of an element, you must continue the ritual by locating a shrine to a trigger. Like the other shrines, the shrine has an obelisk deep within it that will give you the knowledge you need"));
		pages.add(new PlainTextPage("in order to acquire your first trigger rune, which is required for a spell. To bring the pilgrimage to a close, you must locate a Shrine of Self to unlock the shape Self. Upon aquiring knowledge of all components, magic will be unlocked and the ritual completed."));
		...
		pages.add(new PlainTextPage("As part of the Pilgrimage of Discovery, you unlocked knowledge of the elements as well as a trigger and Self runes. However, you only start with mastery of the trigger, Self, and Physical runes. In order to create any other runes, you must perform whichever ritual is required to attain"));
		...
		pages.add(new TitlePage("Rituals", true));
		pages.add(new PlainTextPage("Rituals are the advanced crafting system in Nostrum Magica. Rituals have one or more candles and some number of pedestals. Reagents are burnt in lit candles to form the basis of the Ritual. More complicated rituals take specific items on pedetals."));
		pages.add(new HSplitPage(new PlainTextPage("Candles can be lit by normal means. Reagents can then be used to burn them. Candles do not stay lit forever."), new ImagePage(new ResourceLocation(NostrumMagica.MODID, "textures/blocks/candle.png"), 32, 32, 0, 0, 32, 32, null)));
		pages.add(new HSplitPage(new ImagePage(new ResourceLocation(NostrumMagica.MODID, "textures/items/altar_item.png"), 32, 32, 0, 0, 32, 32, null), new PlainTextPage("Pedestals hold the items used by higher-tier rituals.")));
		pages.add(new PlainTextPage("There are 3 tiers of rituals. Tier 1 has one candle that is surrounded by chalk and can only perform basic rituals. Tier 2 and 3 take both reagents and items and perform much more powerful outcomes, but require pedestals to perform. To construct pedestals, you'll need to find some Kani crystals,"));
		pages.add(new PlainTextPage("which can usually be found as loot in Shrines."));
		pages.add(new PlainTextPage("Regardless of the tier, the layout of candles, chalk, and pedestals is very important. It must exactly match, or the ritual setup cannot be used at all. The layouts of each are as follows:"));
		pages.add(new HSplitPage(new TitlePage("Tier 1", false), new PlainTextPage("Tier 1 is simply a candle surrounded by chalk.")));
		pages.add(new ImagePage(new ResourceLocation(NostrumMagica.MODID, "textures/gui/ritual_1.png"), 96, 96, 0, 0, 96, 96, null));
		pages.add(new HSplitPage(new TitlePage("Tier 2", false), new PlainTextPage("Tier 2 has a pedestal in the center in a diamond of chalk. In each corner is a candle.")));
		pages.add(new ImagePage(new ResourceLocation(NostrumMagica.MODID, "textures/gui/ritual_2.png"), 96, 76, 0, 0, 96, 76, null));
		pages.add(new HSplitPage(new TitlePage("Tier 3", false), new PlainTextPage("Start with tier 2. Surround the candles with chalk and add pedestals 4 blocks out from the center.")));
		pages.add(new ImagePage(new ResourceLocation(NostrumMagica.MODID, "textures/gui/ritual_3.png"), 96, 76, 0, 0, 96, 76, null));
		pages.add(new PlainTextPage("Rituals are performed by burning the required reagents in the candles and (for higher-tier rituals) placing the items on the pedestals. You should mind that your candles don't go out while you're setting up. To actually perform the ritual, a gem infused to the correct element"));
		pages.add(new PlainTextPage("should then be used on the center (candle or altar). The type of gem required is specified by the ritual recipe, which can be viewed at the end of this guide. If everything is in place, the ritual will commence and any effect produced will be granted. All reagents and items used in the ritual will be destroyed."));
