		...
		public void trigger(List<LivingEntity> targets, List<LivingEntity> others, World world, List<BlockPos> locations, boolean forceSplit) {
			...
			if (forceSplit) {
				...
				while ((next = (++index < parts.size() ? parts.get(index) : null)) != null && !next.isTrigger()) {
					...
					LegacySpellShape shape = next.getShape();
...
//			spawn.other = other;
		...
		public EMagicElement getNextElement() {
			for (int i = this.index; i < parts.size(); i++) {
				LegacySpellPart part = parts.get(i);
				if (part.isTrigger()) {
					continue;
				}
	...
	private List<LegacySpellPart> parts;
	private final int manaCost;
	private final int weight;
	
	private LegacySpell(int manaCost, int weight) {
	 ...
	 * @param name
	 */
	public LegacySpell(String name, int manaCost, int weight) {
		this(name, false, manaCost, weight);
	}
	
	public LegacySpell(String name, boolean trans, int manaCost, int weight) {
	...
	public static LegacySpell CreateFake(String name, int id) {
		LegacySpell s = new LegacySpell(0, 0);
		s.name = name;
		s.registryID = id;
	...
	public static LegacySpell CreateAISpell(String name) {
		return new LegacySpell(name, true, 50, 10);
	}
	...
	public LegacySpell addPart(LegacySpellPart part) {
		this.parts.add(part);
		return this;
	}
	...
	public void cast(LivingEntity caster, float efficiency) {
		...
		LegacySpell.onCast(caster, this);
	...
	public String crc() {
		String s = "";
		for (LegacySpellPart part : parts) {
			if (part.isTrigger())
	 ...
	 * @return
	 */
	public static LegacySpell fromNBT(CompoundNBT nbt, int id) {
		if (nbt == null)
			return null;
		...
		LegacySpell spell = new LegacySpell(manaCost, weight);
		spell.name = name;
		spell.registryID = id;
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (tag.contains(NBT_ELEMENT, NBT.TAG_STRING)) {
				LegacySpellShape shape = LegacySpellShape.get(key);
				...
				spell.addPart(new LegacySpellPart(shape, element, count, alteration, param));
	...
	private static void onCast(LivingEntity entity, LegacySpell spell) {
	...
	public Map<LegacySpellShape, Integer> getShapes() {
		Map<LegacySpellShape, Integer> list = new HashMap<>();
		if (!parts.isEmpty())
		for (LegacySpellPart part : parts) {
			if (!part.isTrigger()) {
				int count = 0;
	...
	public List<LegacySpellPart> getSpellParts() {
		return this.parts;
	}
	...
	public boolean shouldTrace() {
		if (!getSpellParts().isEmpty()) {
			LegacySpellPart firstTrigger = getSpellParts().get(0);
